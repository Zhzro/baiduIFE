...
function renderChart() {
    for (var city in chartData) {
        if (city != pageState['nowSelectCity'])
            continue;
        for (var graTime in chartData[city]) {
            if (graTime != pageState['nowGraTime'])
                continue;
            chartWrap.innerHTML = "";
            for (var item in chartData[city][graTime]) {
                // console.log(item + '\n' + graTime[item]);
                var colorLevel = Math.round(chartData[city][graTime][item] / 100);
...
/**
 * 初始化日、周、月的radio事件let，当点击时，调用函数graTimeChange
 */
...
function initCitySelector() {
    ...
    citySelect.onclick = function (event) {
...
function initAqiChartData() {
    ...
    for (var city in aqiSourceData) {
        ...
        chartData[city]['周'] = {};
        chartData[city]['月'] = {};
        var weekDayCount = 0, weekQualityCount = 0;
        var monthDayCount = 0, monthQualityCount = 0;
        var curMonth = 0;
        var firstDayWeek, lastDayWeek;
        var cityKeyData = Object.keys(aqiSourceData[city]);
        for (var i in cityKeyData) {
            ...
            if (new Date(cityKeyData[i]).getDay() == 0 || i == cityKeyData.length - 1) {
                ...
                var avgQuality = parseFloat((weekQualityCount / weekDayCount).toFixed(2));
            ...
            if (new Date(cityKeyData[i]).getMonth() > curMonth || i == cityKeyData.length - 1) {
                var avgQuality = parseFloat((monthQualityCount / monthDayCount).toFixed(2));
